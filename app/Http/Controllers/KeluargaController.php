...
class KeluargaController extends Controller
{
    ...
    {
        $keluarga = Keluarga::with(['pegawai', 'anak']);

        // Filter berdasarkan status pernikahan
        if ($request->filled('status_pernikahan') && $request->status_pernikahan != '') {
            $keluarga->where('status_pernikahan', $request->status_pernikahan);
        }

        // Search nama pegawai
        if ($request->filled('search')) {
            $search = $request->search;
            $keluarga->where(function ($cari) use ($search) {
                $cari->whereHas('pegawai', function ($c) use ($search) {
        ...
        $keluargas = $keluarga->paginate($perPage)->withQueryString();

        return view('cms.admin.kepegawaian.dataKeluarga', compact('keluargas'));
    }

    public function viewDetail($id)
    {
        $keluarga = Keluarga::with(['pegawai', 'anak'])->findOrFail($id);
        if (!$keluarga) {
            return redirect()->route('view-keluarga')->with('error_user', 'Data keluarga tidak ditemukan.');
        }

        return view('cms.admin.kepegawaian.aksi.detailKeluarga', compact('keluarga'));
    }

    public function create()
    {
        $pegawaiBelumPunyaKeluarga = Pegawai::doesntHave('keluarga')->get();

        return view('cms.admin.kepegawaian.aksi.tambahKeluarga', compact('pegawaiBelumPunyaKeluarga'));
    }
    ...
    {
        $keluarga = Keluarga::with(['pegawai', 'anak'])->find($id);
        if (!$keluarga) {
            return redirect()->route('view-keluarga')->with('error_user', 'Data keluarga tidak ditemukan.');
        }

        return view('cms.admin.kepegawaian.aksi.editKeluarga', compact('keluarga'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('view-keluarga')->with('success', 'Data keluarga berhasil diperbarui.');
